@foreach($data as $request)
    <x-molecules.card style="padding: 16px; margin-bottom: 16px; border: 1px solid #e2e8f0; border-radius: 12px; box-shadow: none; background: #ffffff;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 14px; border-bottom: 1px solid #f1f5f9; padding-bottom: 10px;">
            <div>
                <x-atoms.typography variant="body" style="font-weight: 700; color: var(--text-primary);">REQ-#{{ $request->id }}</x-atoms.typography>
                <x-atoms.typography variant="body" style="font-size: 12px; color: var(--text-tertiary);">{{ $request->created_at->translatedFormat('d M Y, H:i') }}</x-atoms.typography>
            </div>
            <div>
        ...
        <div style="display: flex; flex-direction: column; gap: 12px;">
            @foreach($request->details as $detail)
                @php
                    ...
                    $imageUrl = !empty($product->image_path) ? (filter_var($product->image_path, FILTER_VALIDATE_URL) ? $product->image_path : asset('storage/' . $product->image_path)) : $fallbackImage;
                @endphp
                <div style="display: flex; align-items: center; gap: 12px;">
                    ...
                    <div style="flex-grow: 1;">
                        <x-atoms.typography variant="h3" style="font-size: 14px; margin-bottom: 2px;">{{ $product->name ?? 'Produk Tidak Tersedia' }}</x-atoms.typography>
                        <x-atoms.typography variant="body" style="font-size: 12px; color: var(--text-secondary);">Jumlah: {{ $detail->quantity }} pcs &bull; Wajib: {{ $product->mandatory_video_count ?? 1 }} Video</x-atoms.typography>
                    </div>
                </div>
            @endforeach
        </div>
        ...
        <div style="display: flex; justify-content: flex-end; margin-top: 14px; padding-top: 10px; border-top: 1px solid #f1f5f9;">
            <x-atoms.button href="{{ route('affiliator.sample-request.show', $request->id) }}" variant="outline" size="sm" style="border-radius: 6px;">Lihat Detail</x-atoms.button>
        </div>
    </x-molecules.card>
@endforeach